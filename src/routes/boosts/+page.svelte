<script>
	import coin from '$lib/coin.png';
	import colt from '$lib/colt.png';
	import gasoline from '$lib/gasoline.png';
</script>

<div class="absolute left-[115px] top-[180px] inline-flex items-center justify-center gap-2">
	<img class="h-[42px] w-[42px] rounded-[256px] object-cover" alt="coin icon" src={coin} />
	<div class="font-['Roboto'] text-[40px] font-bold leading-[44px] text-[#fffefc]">507,981</div>
</div>
<div class="absolute left-[30px] top-[254px] h-[75px] w-[368px]">
	<div class="absolute left-0 top-0 h-[75px] w-[368px] rounded-[20px] bg-[#101010]"></div>
	<div class="absolute left-[86px] top-[17px] h-[43px] w-[257px]">
		<div class="absolute left-0 top-0 font-['Roboto'] text-sm font-semibold text-white">
			Буст Кольта
		</div>
		<div
			class="absolute left-0 top-[29px] font-['Roboto'] text-xs font-medium leading-3 text-[#ffbd20]"
		>
			Ур. 0
		</div>
		<div
			class="absolute left-[205px] top-[33px] text-right font-['Roboto'] text-[10px] font-medium leading-[10px] text-white"
		>
			+2.5k в час
		</div>
		<div class="absolute left-[201px] top-0 h-[22px] w-14">
			<div class="absolute left-0 top-0 h-[22px] w-14 rounded-md bg-[#ffbd20]"></div>
			<div class="absolute left-[8px] top-[5px] h-3 w-10">
				<img
					class="absolute left-0 top-0 h-3 w-3 rounded-[178.79px] object-cover"
					alt="coin icon"
					src={coin}
				/>
				<div
					class="absolute left-[14px] top-[1px] font-['Roboto'] text-[9.78px] font-semibold leading-[9.78px] text-black"
				>
					14.5K
				</div>
			</div>
		</div>
	</div>
	<img
		class="absolute left-[13px] top-[13px] h-[50px] w-[50px] object-contain"
		alt="colt"
		src={colt}
	/>
</div>

<div class="absolute left-[30px] top-[339px] h-[75px] w-[368px]">
	<div class="absolute left-0 top-0 h-[75px] w-[368px] rounded-[20px] bg-[#101010]"></div>
	<div class="absolute left-[86px] top-[17px] h-[43px] w-[257px]">
		<div class="absolute left-0 top-0 font-['Roboto'] text-sm font-semibold text-white">
			Буст Глоток бензина
		</div>
		<div
			class="absolute left-0 top-[29px] font-['Roboto'] text-xs font-medium leading-3 text-[#ffbd20]"
		>
			Ур. 10
		</div>
		<div
			class="absolute left-[205px] top-[33px] text-right font-['Roboto'] text-[10px] font-medium leading-[10px] text-white"
		>
			+2.5k в час
		</div>
		<div class="absolute left-[195px] top-0 h-[22px] w-[62px]">
			<div class="absolute left-0 top-0 h-[22px] w-[62px] rounded-md bg-[#ffbd20]"></div>
			<div class="absolute left-[8px] top-[5px] h-3 w-[46px]">
				<img
					class="absolute left-0 top-0 h-3 w-3 rounded-[178.79px] object-cover"
					alt="coin icon"
					src={coin}
				/>
				<div
					class="absolute left-[14px] top-[1px] font-['Roboto'] text-[9.78px] font-semibold leading-[9.78px] text-black"
				>
					140.5K
				</div>
			</div>
		</div>
	</div>
	<img
		class="absolute left-[13px] top-[13px] h-[50px] w-[50px] object-contain"
		alt="gasoline"
		src={gasoline}
	/>
</div>
